...
<?php
$result = mysqli_query($con,"SELECT sum(amount) FROM  deposit where username = '".$_SESSION['login_user']."'");
...
?>
<?php
$result = mysqli_query($con,"SELECT count(*) FROM  users where level=1");
$row = mysqli_fetch_row($result);
$numrows = $row[0];

?>
<?php
$query="SELECT  sum(amount) FROM referal where username = '".$loggedin_session."'";
$result = mysqli_query($con,$query);
while($row = mysqli_fetch_array($result))
{
    $refer=$row[0];
